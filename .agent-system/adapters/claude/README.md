...
All 15 canonical agents (count corrected 2026-09-22 — `orchestrator` was previously left out of this count, it maps to the `dev-workflows` skill row above, not a `.claude/agents/` file) now have either a real `.claude/` dispatch file, a real `.claude/skills/*/SKILL.md`, or (Ponytail) a real installed plugin — every one auto-dispatches in Claude Code without the user asking for it by name. The files in the right-hand column are kept in sync by whoever edits the canonical file; this adapter README only documents the mapping and the sync convention, it does not itself edit those files.
...
- **MCP integrations beyond the above** — a large set of `claude.ai`-hosted connectors (Notion, Slack, Figma, Asana, Box, Canva, HubSpot, Intercom, Linear, Microsoft 365, monday.com, GitBook, Claude Docs) that are account-level, not project-specific, and not currently wired into this project's workflows. **Correction 2026-09-22**: a prior version of this line listed a Supabase-homolog MCP tool group (`mcp__supabase-homolog__*`) — Supabase was fully removed from this project 2026-09-21 and no such tool is present in current sessions; removed as stale.
