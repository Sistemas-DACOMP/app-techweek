...
- **Antigravity**: use `invoke_subagent` com o conteúdo de `.agent-system/agents/<id>.md`. Antigravity não tem ferramenta de todo-list — materialize `.agent-system/templates/task-context.md` como arquivo real via `write_to_file`, não como checklist em memória. Ver `.agent-system/adapters/antigravity/README.md` — também não testado ponta a ponta nesta máquina.
