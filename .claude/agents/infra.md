...
- `firebase.json`, `firestore.rules`, `.firebaserc` já existem na raiz do repo, apontando pro projeto `facom-techweek-layerx`. `firestore.indexes.json` e `storage.rules` ainda não existem — não confunda "CLI disponível" com "infraestrutura completa": ainda falta criar ambos.
