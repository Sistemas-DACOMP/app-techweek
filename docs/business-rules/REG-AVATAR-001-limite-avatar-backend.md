...
prioridade: media
status: correção em PR (migration `supabase/migrations/0004_avatar_storage_limits.sql`, ainda não aplicada em nenhum projeto Supabase — precisa ser rodada manualmente em homolog e depois produção)
...
implementacao_relacionada: src/pages/Profile.jsx, uploadAvatar, supabase/migrations/0004_avatar_storage_limits.sql
...
**2026-09-13 (revisão do PR #23)**: migration renomeada de `0003` pra `0004_avatar_storage_limits.sql` pra não colidir com a numeração do PR #22 (`0003_lgpd_terms_required.sql`). Também corrigido um mismatch entre front e Storage: `validateAvatarFile` aceitava qualquer `image/*`, mas a migration restringe o Storage a 4 tipos fixos (png/jpeg/webp/gif) — um arquivo `image/svg+xml`, `image/bmp` ou `image/heic` passava no front e só era recusado no upload, com erro genérico. `validateAvatarFile` agora checa contra a mesma lista explícita (`ALLOWED_AVATAR_MIME_TYPES`), e a mensagem de erro no front foi ajustada pra citar os tipos aceitos.
